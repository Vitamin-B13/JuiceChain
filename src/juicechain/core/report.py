...
def build_scan_report(scan_data: Mapping[str, Any], vuln_data: Mapping[str, Any] | None = None) -> str:
    ...
    alive = scan_doc.get("alive") if isinstance(scan_doc.get("alive"), Mapping) else {}
    info = scan_doc.get("info") if isinstance(scan_doc.get("info"), Mapping) else {}
    enum = scan_doc.get("enum") if isinstance(scan_doc.get("enum"), Mapping) else {}

    homepage = info.get("homepage") if isinstance(info.get("homepage"), Mapping) else {}
    fingerprint = info.get("fingerprint") if isinstance(info.get("fingerprint"), Mapping) else {}
    security_headers = (
        info.get("security_headers") if isinstance(info.get("security_headers"), Mapping) else {}
    )

    crawler = enum.get("crawler") if isinstance(enum.get("crawler"), Mapping) else {}
    spa = crawler.get("spa") if isinstance(crawler.get("spa"), Mapping) else {}
    content_discovery = (
        enum.get("content_discovery") if isinstance(enum.get("content_discovery"), Mapping) else {}
    )
    ...
    if vuln_data is not None:
        ...
        if findings:
            ...
            for finding in _sort_findings(findings):
                request = (
                    finding.get("request") if isinstance(finding.get("request"), Mapping) else {}
                )
                response = (
                    finding.get("response") if isinstance(finding.get("response"), Mapping) else {}
                )
                lines.append(
                    "| "
                    + " | ".join(
